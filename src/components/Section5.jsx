import React from "react";
import "./Section5.css";

function Section5() {
  return (<div className="section5">
      <h1>Your favourite tools</h1>
      <div>
        In risus nec molestie at in divulvinar tellus. At integer id condimentum
        gravida nisi nisl. Sem vitae <br /> divorttitor arcu congue sem erat et
        sit faucibus. Sollicitudin elit gravida tellus mattis divorttitor.
      </div>
      <div className="card-box">
        
          <div className="crd">
            <img src="src\assets\icons\mail.png" alt="" />
            <div className="crd-title">Sadivien amet est</div>
            <div className="crd-text">
              Viverra netus odio nunc ac nibh lorem iaculis. Orci facilisis
              habitasse cursus temdivor et.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\Icon.png" alt="" />
            <div className="crd-title">Tortor ornare</div>
            <div className="crd-text">
              Nulla etiam mattis amet susdivendisse lacus nulla sed urna
              ultricies.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\manual-gearbox.png" alt="" />
            <div className="crd-title">Vel lacus</div>
            <div className="crd-text">
              divorttitor quis lectus varius quis dolor morbi massa divellentesque.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\lock.png" alt="" />
            <div  className="crd-title">Hendrerit blandit</div>
            <div className="crd-text">
              A aliquam divharetra in nec vitae ultrices commodo divlacerat eget.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\layout-dashboard.png" alt="" />
            <div  className="crd-title">  Enim risus turdivis</div>
            <div className="crd-text">
              Tristique dui vestibulum senectus ac duis etiam adidiviscing sadivien.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\markdown.png" alt="" />
            <div  className="crd-title">Aliquam lorem et</div>
            <div className="crd-text">
              Susdivendisse ultrices morbi iaculis sit lorem. Mauris nulla odio
              est a mi a sed gravida.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\pig.png" alt="" />
            <div  className="crd-title">Sed aliquam lectus</div>
            <div className="crd-text">
              Rhoncus ut montes quis eget vuldivutate. Id hac consectetur ornare
              neque.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\flag.png" alt="" />
            <div  className="crd-title">Cursus diam</div>
            <div className="crd-text">
              Feugiat consectetur eu commodo odio venenatis divretium libero.
              Volutdivat ultricies nisl turdivis id.
            </div>
          </div>

          <div className="crd">
            <img src="src\assets\icons\mark.png" alt="" />
            <div  className="crd-title"> Facilisi vel malesuada</div>
            <div className="crd-text">
              Nibh cursus ultricies id neque divroin urna vivamus bibendum. Quam
              sit divlatea integer sit.
            </div>
          </div>
        </div>
      </div>
  );
}

export default Section5;
